...
const stats = [
  { label: 'Hospital Beds', value: '1.9M', sub: '1.4 per 1000', icon: Building2, color: 'text-primary' },
...
const HealthcareIndicators: React.FC = () => (
  <DashboardLayout>
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-gray-900 flex items-center"><Heart className="h-6 w-6 mr-2 text-red-500" />Healthcare Indicators</h2>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map(s => { const Icon = s.icon; return (
          <motion.div key={s.label} whileHover={{ y: -2 }} className="bg-white rounded-xl border border-gray-200 p-4 shadow-card">
            <Icon className={`h-8 w-8 ${s.color} mb-2`} /><p className="text-2xl font-bold text-gray-900">{s.value}</p><p className="text-sm text-gray-500">{s.label}</p><p className="text-xs text-gray-400">{s.sub}</p>
          </motion.div>
        )})}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-card">
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={stateHealth}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="state" /><YAxis /><Tooltip /><Bar dataKey="beds" fill="#1E40AF" name="Beds/1000" /><Bar dataKey="doctors" fill="#10B981" name="Doctors/1000" /></BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-card">
          <h3 className="text-lg font-semibold mb-4">Disease Surveillance</h3>
          <ResponsiveContainer width="100%" height={300}>
            <LineChart data={diseaseData}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Line type="monotone" dataKey="dengue" stroke="#EF4444" strokeWidth={2} /><Line type="monotone" dataKey="malaria" stroke="#F59E0B" strokeWidth={2} /><Line type="monotone" dataKey="covid" stroke="#8B5CF6" strokeWidth={2} /></LineChart>
          </ResponsiveContainer>
        </div>
      </div>
    </div>
