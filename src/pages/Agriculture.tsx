...
const Agriculture: React.FC = () => (
  <DashboardLayout>
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white flex items-center"><Wheat className="h-6 w-6 mr-2 text-amber-600" />Agriculture Dashboard</h2>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[{ l: 'Foodgrain Output', v: '330 MT', i: Wheat, c: 'text-amber-600' }, { l: 'Monsoon Status', v: '-8% Deficit', i: CloudRain, c: 'text-blue-600' }, { l: 'Food Inflation', v: '7.1%', i: IndianRupee, c: 'text-red-500' }, { l: 'Agri GDP Share', v: '15.2%', i: TrendingUp, c: 'text-green-600' }].map(s => { const Icon = s.i; return (
          <div key={s.l} className="bg-cyber-card rounded-xl border border-cyber-border p-4 shadow-card"><Icon className={`h-8 w-8 ${s.c} mb-2`} /><p className="text-2xl font-bold text-white">{s.v}</p><p className="text-sm text-gray-500">{s.l}</p></div>
        )})}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="bg-cyber-card rounded-xl border border-cyber-border p-6 shadow-card">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={crops}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="crop" tick={{ fontSize: 11 }} /><YAxis /><Tooltip /><Bar dataKey="production" fill="#FFB800" radius={[4,4,0,0]} /></BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-cyber-card rounded-xl border border-cyber-border p-6 shadow-card">
          <h3 className="text-lg font-semibold mb-4">Rainfall: Actual vs Normal (mm)</h3>
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={rainfall}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Area type="monotone" dataKey="normal" fill="#93C5FD" stroke="#00F0FF" fillOpacity={0.3} /><Area type="monotone" dataKey="actual" fill="#00FF88" stroke="#059669" fillOpacity={0.3} /></AreaChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-cyber-card rounded-xl border border-cyber-border p-6 shadow-card">
          <h3 className="text-lg font-semibold mb-4">Food vs Overall Inflation (%)</h3>
          <ResponsiveContainer width="100%" height={280}>
            <LineChart data={inflation}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="month" /><YAxis /><Tooltip /><Line type="monotone" dataKey="food" stroke="#FF3366" strokeWidth={2} name="Food" /><Line type="monotone" dataKey="overall" stroke="#00F0FF" strokeWidth={2} name="Overall" /></LineChart>
          </ResponsiveContainer>
        </div>
      </div>
    </div>
