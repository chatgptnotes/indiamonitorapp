...
const COLORS = ['#00F0FF', '#00FF88', '#FFB800', '#FF3366', '#8B5CF6', '#6B7280']

const CrimeStatistics: React.FC = () => (
  <DashboardLayout>
    <div className="space-y-6">
      <h2 className="text-2xl font-bold text-white flex items-center"><ShieldAlert className="h-6 w-6 mr-2 text-danger" />Crime Statistics</h2>
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[{ l: 'Crime Rate', v: '55.8L', d: 'Total cases 2025', up: false }, { l: 'Cyber Crime', v: '+34%', d: 'YoY increase', up: true }, { l: 'Conviction Rate', v: '52.4%', d: '+2.1% from 2024', up: true }, { l: 'Pending Cases', v: '4.7Cr', d: 'In courts', up: true }].map(s => (
          <div key={s.l} className="bg-cyber-card rounded-xl border border-cyber-border p-4 shadow-card">
            <p className="text-2xl font-bold text-white">{s.v}</p><p className="text-sm text-gray-500">{s.l}</p><p className="text-xs text-gray-400 flex items-center">{s.up ? <TrendingUp className="h-3 w-3 mr-1 text-red-500" /> : <TrendingDown className="h-3 w-3 mr-1 text-green-500" />}{s.d}</p>
          </div>
        ))}
      </div>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-cyber-card rounded-xl border border-cyber-border p-6 shadow-card">
          ...
          <ResponsiveContainer width="100%" height={250}>
            <BarChart data={stateData} layout="vertical"><CartesianGrid strokeDasharray="3 3" /><XAxis type="number" /><YAxis dataKey="state" type="category" width={90} /><Tooltip /><Bar dataKey="rate" fill="#FF3366" radius={[0,4,4,0]} /></BarChart>
          </ResponsiveContainer>
        </div>
        <div className="bg-cyber-card rounded-xl border border-cyber-border p-6 shadow-card">
          <h3 className="text-lg font-semibold mb-4">Crime vs Cyber Crime Trend</h3>
          <ResponsiveContainer width="100%" height={250}>
            <LineChart data={trends}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="year" /><YAxis /><Tooltip /><Line type="monotone" dataKey="total" stroke="#00F0FF" name="Total (Lakhs)" /><Line type="monotone" dataKey="cyber" stroke="#FF3366" name="Cyber (Lakhs)" /></LineChart>
          </ResponsiveContainer>
        </div>
      </div>
    </div>
