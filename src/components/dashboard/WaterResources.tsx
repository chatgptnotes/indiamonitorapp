    ...
    <div className="space-y-2">
      {reservoirs.map(r => (
        <div key={r.name} className="flex items-center space-x-2">
          <span className="text-xs text-gray-400 w-24 truncate" title={`${r.name}, ${r.state}`}>{r.name}</span>
          <div className="flex-1 h-3 bg-cyber-border/30 rounded-full"><div className={`h-full rounded-full ${getLevelColor(r.level)}`} style={{ width: `${r.level}%` }} /></div>
          <span className="text-xs font-semibold text-gray-300 w-8">{r.level}%</span>
        </div>
      ))}
    </div>
  </div>
