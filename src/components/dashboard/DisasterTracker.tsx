...
const DisasterTracker: React.FC = () => (
  <div className="space-y-3">
    <h4 className="text-sm font-semibold text-gray-700 flex items-center"><AlertTriangle className="h-4 w-4 mr-1 text-danger" />Disaster Tracker</h4>
    {disasters.map((d, i) => {
      const Icon = d.icon
      return (
        <motion.div key={i} initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: i * 0.1 }} className="bg-white rounded-lg border border-gray-200 p-3 flex items-center space-x-3">
          <div className={`p-2 rounded-lg ${d.color}`}><Icon className="h-4 w-4" /></div>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-gray-900">{d.type} - {d.location}</p>
