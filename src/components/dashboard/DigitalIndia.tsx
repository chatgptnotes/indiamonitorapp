...
const DigitalIndia: React.FC = () => (
  <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-card">
    <h3 className="text-base font-semibold mb-3 flex items-center"><Wifi className="h-5 w-5 mr-2 text-primary" />Digital India</h3>
    <div className="grid grid-cols-2 gap-2 mb-3">
      {[{ l: 'Broadband', v: '920M', i: Wifi, c: 'text-blue-600 bg-blue-50' }, { l: 'UPI/month', v: '14.8B', i: CreditCard, c: 'text-green-600 bg-green-50' },
        ...
        <div key={s.l} className={`p-2 rounded-lg ${s.c.split(' ')[1]}`}>
          <Icon className={`h-4 w-4 ${s.c.split(' ')[0]} mb-1`} /><p className="text-lg font-bold text-gray-900">{s.v}</p><p className="text-[10px] text-gray-500">{s.l}</p>
        </div>
      )})}
    </div>
    <p className="text-xs text-gray-500 mb-1">UPI Transactions (Billions)</p>
    <ResponsiveContainer width="100%" height={100}>
      <AreaChart data={upiData}><XAxis dataKey="month" tick={{ fontSize: 9 }} /><Tooltip /><Area type="monotone" dataKey="txns" stroke="#1E40AF" fill="#1E40AF" fillOpacity={0.15} name="Txns (B)" /></AreaChart>
    </ResponsiveContainer>
  </div>
)
